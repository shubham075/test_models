...
> **Technology**: React + Vite (Frontend) | ASP.NET Core (Backend API) | SQL Server: `IOMS`  
> **Manual Version**: 1.1 (Database Schema Verified — 05-Jun-2026)  
> **Session Timeout**: 95 minutes of inactivity (JWT expiry)
...
FlowDesk supports **three primary roles**, stored in the `[dbo].[AspNetRoles]` table (`IOMS` database). Each role has database-level capability flags:

| DB Role Flag | Description |
|---|---|
| `CanForwardAction` | Can forward inward letters to another officer |
| `CanReviewOutward` | Can be selected as a reviewer for outward letters |
| `CanApproveOutward` | Can approve outward letters in the approval chain |

Frontend role detection (from `authSlice.js`) is a fallback based on username pattern:
...
| Forward Outward for Review | ✅ | `CanReviewOutward` | `CanReviewOutward` |
| Approve Outward | ✅ | `CanApproveOutward` | `CanApproveOutward` |
| Send / Dispatch Outward | ✅ | ✅ | ✅ |
| Pending Actions (Acknowledge / Forward / Reply) | ✅ | ✅ | ⚠️ per API |
| Forward Inward to Officer | ✅ | `CanForwardAction` | `CanForwardAction` |
...
3. Choose login method:
   - **Password Login**: Enter your **username** and **password**
   - **OTP Login**: Enter your registered **Username** to receive an OTP (6-digit code sent via SMS)
...
> **OTP Details** (from `User_Otp` table): OTPs are 6-digit numeric codes, linked to your `UserName` and `IPAddress`. Each OTP has an expiry timestamp (`ExpiryDate`). Consumed OTPs are archived to `User_Otp_History` with a `VerifiedAt` timestamp.

### Session Management
- Sessions expire after **95 minutes** from login (per the JWT `exp` claim)
...
3. **Fill Letter Metadata** (right panel — unlocks after file upload):
   - **Department Name** *(required)*: Select from the dropdown (loaded from API → `Departments` table)
   - **Service Name** *(required)*: Select a service (loaded based on selected department → `Services` table)
   - **Letter Number** *(required)*: e.g., `MOF/IN/2026/049` (stored as `SenderReferenceNo` in DB)
   - **Sender / Received From** *(required)*: Name of the sending officer/organization (stored as `ReceivedFrom` in DB)
   - **Sender Organization** *(optional)*: Organization name of the sender (stored as `SenderOrganization` in DB)
   - **Letter Date** *(required)*: The date on the letter (stored as `LetterDate`)
   - **Letter Received Date** *(required)*: The date your office received it (stored as `LetterReceivedDate`)
   - **Letter Subject** *(required)*: Brief description of the letter content (stored as `Subject`, max 500 chars)
   - **Urgency Level** *(required)*: `Normal` or `Urgent` (stored as `UrgencyLevel` in DB — controls priority badge color)
...
> **DB Note**: Registered letters are stored in `InwardLetters` with `CurrentStatus = 'Registered'` or similar initial status. The `RegisteredBy` field captures the `UserName` of the clerk who submitted. The system also assigns the letter to the department hierarchy: `AssignedDHID` → `AssignedSHID` → `AssignedBLID` → `AssignedBAID`.
...
2. Fill the **Outward Registry Form**:
   - **Recipient Department** *(required)*: Select from dropdown (loaded from API → `Departments` table)
   - **Recipient Service** *(required)*: Select after choosing department (→ `Services` table)
   - **Addressed To** *(required)*: Name of the receiving officer (`AddressedTo` in DB, max 300 chars)
   - **Addressed To Organization** *(optional)*: Organization/office address (`AddressedToOrganization` in DB)
   - **Subject** *(required)*: Content description, max 500 characters (stored as `Subject`)
   - **Outward Type** *(required)*: Select dispatch mode (e.g., Speed Post, Courier — from `OutwardLetterType` lookup table)
   - **Urgency Level** *(required)*: `Normal` or `Urgent` (stored as `UrgencyLevel`)
   - **Remarks** *(optional)*: Initial notes or references (stored as `InitialRemark`)
3. The **Outward Type** panel (right side) shows either:
   - `New Outward Mail` — fresh letter
   - `Reply to Inward` — auto-filled when navigated from Pending Actions "Reply" button (sets `InwardLetterID` FK in DB)
4. Click **Save Outward Mail**.
5. On success: a popup modal shows the generated **Outward Letter Number** (`OutwardLetterNo` in DB)
...
This section lists the key `IOMS` database tables that back each FlowDesk feature, for developer reference.

| FlowDesk Feature | Primary Table(s) | Notes |
|---|---|---|
| User Authentication | `AspNetUsers`, `AspNetRoles`, `AspNetUserRoles` | `EmpId` is the unique employee code |
| OTP Login | `User_Otp`, `User_Otp_History` | Supports Mobile OTP and Email OTP via `OtpType` |
| Password History | `PasswordHistory` | Stores hashed old/new passwords |
| Departments | `Departments` | `DepartmentCode` used as letter prefix |
| Services | `Services` | Each service belongs to one department |
| Service-Dept Mapping | `ServiceTeamMapping`, `UserDepartmentServiceMapping` | Maps officers (HOD/SH/BL/BA) to service-dept pairs |
| Employee Hierarchy | `mst_emp_mapping` | DH → SH → BL → BA assignment chain per department |
| Inward Letters | `InwardLetters` | `CurrentStatus` (nvarchar) tracks lifecycle |
| Inward Actions Audit | `InwardLetterActions` | Every action logged with `ActionType`, `Remark`, `ActionAt` |
| Outward Letters | `OutwardLetters` | `CurrentStatus` (int): 10=Prepared, 20=In Approval, 30=Under Review, 40=Dispatched |
| Outward Actions Audit | `OutwardLetterActions` | `ActionType` is a string (e.g., `"UPLOAD"`, `"FORWARD"`, `"APPROVE"`) |
| Outward Reviewers | `OutwardReviewers` | Supports multi-reviewer chains via `ReviewOrder` |
| Document Storage | `LetterDocuments` | `LetterType` = `'Inward'` or `'Outward'`; `FilePath` = server path |
| Notifications | `Notifications`, `NotificationType` | `IsRead` bit tracks read/unread status |
| Email Notifications | `EmailLogs` | Records every email sent; linked to letters via `RelatedLetterID` |
| Error Logging | `ErrorLogs` | API-level error capture for backend debugging |

### Officer Assignment Hierarchy (Inward Letters)

When an inward letter is registered, it is assigned through a 4-level hierarchy stored in `InwardLetters`:

```
DH (Divisional Head)  →  AssignedDHID
  └── SH (Section Head)  →  AssignedSHID
        └── BL (Branch Level Officer)  →  AssignedBLID
              └── BA (Branch Admin)  →  AssignedBAID
```

The `CurrentActionOwnerID` tracks who is responsible for the letter at any given moment.

---

*Document generated from source analysis of FlowDesk React frontend & IOMS database schema — June 2026*  
*Version 1.1 — Database Schema Verified 05-Jun-2026*
